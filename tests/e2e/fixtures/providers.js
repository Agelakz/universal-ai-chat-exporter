...
const chatgptConversation = () => ({
  ...
  messages: alternatingMessages().map((message, index) => ({
    id: message.id,
  ...
  page_info: { start_cursor: "message-0", end_cursor: "message-119", has_previous_page: false, has_next_page: false }
});

const geminiRpcBody = () => {
  const turns = alternatingMessages().reduce((result, message, index) => {
    if (message.role !== "user") return result;
    const assistant = alternatingMessages()[index + 1];
    const turn = [];
...
module.exports = { LONG_MESSAGE_COUNT, chatgptConversation, geminiRpcBody, claudeConversation, domConversationHtml };
